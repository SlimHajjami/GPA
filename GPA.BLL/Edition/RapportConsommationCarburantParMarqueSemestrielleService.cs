...
namespace GPA.BLL
{
    public class RapportConsommationCarburantParMarqueSemestrielleService
    {
        ...
        public static List<RapportConsommationCarburantParMarqueSemestrielleModel> Read(int client, DepartementsAccessRightsModel departementsAccess, int annee, int mois)
        {
            using (Entities context = new Entities())
            {
                DateTime currentMonth = new DateTime(annee, mois, 1);

                List<RapportConsommationCarburantParMarqueSemestrielleModel> list = new List<RapportConsommationCarburantParMarqueSemestrielleModel>();
                ...
                if (vehicules != null)
                {
                    ...
                    {
                        string marque = "";
                        var obj = context.REFERENTIELs.Where(x => x.KEY_REF == "MARQUE_VEHICULE" && x.CODE_REF == v.codeRefMarque).FirstOrDefault();
                        if (obj != null)
                            marque = obj.VALUE;

                        RapportConsommationCarburantParMarqueSemestrielleModel row = new RapportConsommationCarburantParMarqueSemestrielleModel
                        {
